...
CBSA = c("Portland-Vancouver")
CENTRAL_CITY = c("Portland city")
...
#Removing the CDP tracts from Portland
...
#For initial visualizing of Portland
Portland2011 <- ggplot() + 
...
```{r, fig.cap = "Figure 1. Portland's census inspired suburbs in 2011 and 2021"}
Portland2011 + Portland2021 + plot_layout(widths = c(6, 6), guides = "collect") & theme(legend.position = "bottom")
...
#To visualize Portland
Portland2011IO <- ggplot() + 
...
```{r fig.cap = "Figure 2. Portland's Inner and Outer suburbs in 2011 and 2021"}
Portland2011IO + Portland2021IO + plot_layout(widths = c(6, 6), guides = "collect") & theme(legend.position = "bottom")
```
...
#For genereal visualization of Portland, NC
Portland2011PP <- ggplot() + 
...
```{r fig.cap = "Figure 3. Portland's Pre- and Post-Civil Rights suburbs in 2011 and 2021"}
...
ggplot(Pop_Totals[Pop_Totals$City == "Portland", ], 
  ...
  geom_text(aes(x = Estimate, y = Suburb, label = scales::percent(PercentChange)), 
            hjust = 0, size = 4, color = "black", fontface = "bold", alpha = 1,
